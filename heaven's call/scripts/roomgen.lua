...
function mod:InitializeRoomData()
	local level = game:GetLevel()
	local currentroomidx = level:GetCurrentRoomDesc().SafeGridIndex
	...
	for i = mod.minvariant, mod.maxvariant, 1 do
		Isaac.ExecuteCommand("goto s.dice."..i)
		mod.roomdata[i] = level:GetRoomByIdx(-3,0).Data--I dont know what the -3 does
	end
